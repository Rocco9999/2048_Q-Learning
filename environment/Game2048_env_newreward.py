...
class Game2048_env(gym.Env):
    ...
    def calculate_reward2(self, score, valid, game_over, max_number, alpha=0.1):
        reward = 0

        tile_values = [tile for row in self.game.moved_board for tile in row if tile > 0]
        tile_values_prev = [tile for row in self.game.board for tile in row if tile > 0]

        diff_value = sum(tile_values) - sum(tile_values_prev)
        
        bonus = np.log2(diff_value) if diff_value else 0

        if not valid:
            reward = -alpha * bonus
        elif valid and score > 0:
            reward = bonus
        else:
            reward = 0
